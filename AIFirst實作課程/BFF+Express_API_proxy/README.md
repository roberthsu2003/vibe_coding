...
```markdown
請用 React + TypeScript + Vite 建立專案，並符合下列架構：
...
- **送出機制**：
  - 一個「送出」按鈕。
  - 點擊後由前端呼叫我們自己的後端 API（BFF 代理）。
  - 等待 API 回應期間，需顯示「載入中」狀態或將按鈕設為停用（Disabled）。
- **結果呈現**：
  - **成功時**：在輸入區下方顯示 Gemini 回覆的純文字。
  - **失敗時**：顯示簡短的錯誤提示訊息。
- 🔒 **安全性限制**：應用程式畫面及瀏覽器 Network 請求紀錄中，**皆不得暴露 `GEMINI_API_KEY`**。

### 📦 輸出要求
1. 請分檔提供關鍵的實作程式碼。
2. 請簡述在本機環境中，如何分別啟動「前端專案」與「測試用後端 API 伺服器」。
```
