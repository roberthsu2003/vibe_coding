...
```markdown
請用 React + TypeScript + Vite 建立專案，並導入 BFF (Backend For Frontend) 模式與 Express API proxy 方式，符合下列架構：

### 🛠 技術棧
- **前端：** Vite + React + TypeScript（與常見 `react_typescript_vite` 專案慣例相容即可）
- **後端：** Node.js + Express（作為 API Proxy 代理伺服器）
...
- **送出機制**：
  - 一個「送出」按鈕。
  - 點擊後由前端呼叫我們自己的 Express 後端 API（BFF 代理）。
  - 等待 API 回應期間，需顯示「載入中」狀態或將按鈕設為停用（Disabled）。
- **結果呈現**：
  - **成功時**：在輸入區下方顯示 Gemini 回覆的純文字。
  - **失敗時**：顯示簡短的錯誤提示訊息。
- 🔒 **安全性限制**：應用程式畫面及瀏覽器 Network 請求紀錄中，**皆不得暴露 `GEMINI_API_KEY`**。API Key 只能在後端伺服器安全被讀取與呼叫。

### 📦 輸出要求
1. 請分檔提供關鍵的實作程式碼（需包含前端與 Express 後端程式碼）。
2. 請簡述在本機環境中，如何分別啟動「前端專案」與「測試用 Express 後端 API 伺服器」。
```
### 提供的實際專案
...
### 在 AI 編輯器開啟此專案並轉換為 BFF 架構

**使用AI編輯器的Prompt,將google ai studio產生的react專案轉換為BFF架構**
