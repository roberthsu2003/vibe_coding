...
2. **進階：使用 `curl` 測試新增 (POST 請求)**：
   - **如何測試**：如果您熟悉終端機 (Terminal / Command Prompt)，可以複製以下指令，並將 `[請填入你的 GAS 網址]` 替換為您的網址後執行（**⚠️ 提醒：必須加上 `-L` 參數以跟隨 Google Apps Script 的重定向**）：
     ...
     ```
   - **正確的結果**：您的終端機應該會回傳類似下面的成功 JSON，同時您會發現 Google 試算表左下角**神奇地自動產生**了一個以今天日期命名的分頁（如 `2026-05-24`），裡面整齊地記錄了「張小飛」的訂單！
     ```json
     {"status":"success","message":"訂單已成功記錄至工作表 2026-05-24","orderId":"uuid-string"}
     ```
