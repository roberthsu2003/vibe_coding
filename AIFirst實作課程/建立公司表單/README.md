...
> [!WARNING]
> **⚠️ 修改 GAS 程式碼的大魔王注意事項（新手最容易踩的坑）：**
> 如果您未來修改了 Google Apps Script 中的任何程式碼（例如修正了某個 Bug 或是增加了新功能），**僅僅點擊「儲存」是絕對不會在線上生效的！** 
> 
> 您必須透過「更新部署」來讓新程式碼生效，且**必須嚴格遵守以下步驟，否則線上跑的永遠是舊程式碼**：
> 1. 點擊右上角「部署」 > 「**管理部署作業**」。
> 2. 點擊右上角的 **「編輯」** (鉛筆圖示 ✏️)。
> 3. **關鍵大招（必做）**：在「版本」下拉選單中，**一定要選擇「建立新版本」**！
>    *(⚠️ 如果沒有點選「建立新版本」，即使您點了部署，Google 依然只會執行您上一次的舊代碼，您的任何修改都不會生效！)*
> 4. 點擊右下角的 **「部署」**。
> 
> 🛑 **千萬不要**每次都點選「新增部署作業」！如果點了新增部署，會產生一個「全新且不同」的 API 網址，這樣您原先放在網頁裡的網址就會直接失效！
...
1. **瀏覽器測試 (GET 請求)**：
   ...
   - **正確的結果**：您應該會看到瀏覽器輸出類似下方的純 JSON 資料（包含您在 `Menu` 建立的 10 個商品項目，且 `orders` 預設為空）：
     ```json
...
2. **進階：使用 `curl` 測試新增 (POST 請求)**：
   - **如何測試**：如果您熟悉終端機 (Terminal / Command Prompt)，可以複製以下指令，並將 `[請填入你的 GAS 網址]` 替換為您的網址後執行（**⚠️ 提醒：必須加上 `-L` 參數以跟隨 Google Apps Script 的重定向**）：
     ...
     curl -L -X POST "[請填入你的 GAS 網址]" \
       ...
       -d '{"action": "create", "data": {"name": "張小飛", "drink": "烏龍拿鐵", "sugar": "微糖", "ice": "少冰", "quantity": 2, "totalPrice": 120}}'
     ```
   - **正確的結果與⚠️特別注意（極度重要）**：
     當您在終端機執行上述指令後，畫面上**非常有可能會噴出一大段包含「很抱歉，目前無法開啟這個檔案」的錯誤 HTML 代碼**。
     
     > [!WARNING]
     > **這不是您的 API 寫壞了！** 這是因為 Google Apps Script 的安全重定向防禦機制——當 API 在後端寫入成功後，會將結果重定向 (302 Found) 到一個暫時的 Google 雲端網址。此時匿名的 `curl` 命令在跟隨重定向時，會被 Google 的安全網關攔截而回傳該錯誤頁面。
     
     **💡 如何確認是否成功？**
     請**立刻直接打開您的 Google 試算表**！如果您發現左下角**神奇地自動產生**了一個以當天日期命名的分頁（如 `2026-05-24`），且裡面第一行是美化好的表頭，表頭下方已經整整齊齊地寫入了「張小飛」的訂單資料，**那就代表您的 API 已經 100% 完美成功運行！** 
     *(將來我們在瀏覽器中雙擊打開網頁前端時，前端的 JavaScript 會自動處理好 CORS 跨網域通訊，完全不會遇到這個 curl 工具專屬的重定向安全限制)*。
...
> 2. OrderList 元件：精美的當日訂單列表。顯示今天所有已訂購的項目、甜度冰塊、數量與總金額。並提供「編輯」與「刪除」按鈕，點擊編輯時能將資料填回 OrderForm；點擊刪除時呼叫 POST API。**如果當日一開始沒有任何人訂購，必須在列表區域顯眼且精美地提示「目前無人訂購」，以引導大家開始點單。**
