...
- **Code（程式碼）模式**：需**切換到 Code 模式**後，才會出現**下載專案**（或匯出／打包下載等）相關操作；**要備份到本機或上傳到 GitHub，請先進入 Code 模式再下載**。

介面上的切換按鈕名稱、位置與下載選項可能隨改版調整，以當前 AI Studio 為準。
...
- 於本資料夾新增專案檔、匯出程式或 Google AI Studio 相關設定備註（下載請在 **Code 模式**操作，勿只在 Preview 畫面找下載）。
