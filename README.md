...
1. **產品經理 / 非技術人員 (PM / Non-tech)**：目標多半是快速做出 MVP、原型或對外 demo，程式能力不是強項。
   - *建議路徑*：以 **AI First** 為主（迭代思維、Prompt Engineering、自然語言驅動開發）。**AI Assisted** 區塊裡的 Git／GitHub 可先學「能備份與還原」的最小集合；**SDD**、**TDD** 等偏工程規格與測試的內容，建議只保留概念或極度簡化，避免被細節拖住產出節奏。
2. **程式新手 (Beginners)**：已接觸基礎語法，希望藉 AI 獨立完成或維護小型專案。
   - *建議路徑*：可先用 **AI First** 建立「能交付」的信心與節奏，再逐步銜接 **AI Assisted**（先修、規格文件、PRD／Todo、SDD）。**TDD** 對新手門檻高，可務實轉成「讓 AI 協助撰寫測試、跑測試與協助除錯」，再視需要加深。
...
完整主題列表、各資料夾用途，以及每個主題內 **課堂規劃（約）／作品／實作放置** 的說明，請見 **[`AIFirst實作課程/README.md`](./AIFirst實作課程/README.md)**；點入任一模組資料夾即可對照該堂內容與繳交檔案位置。
